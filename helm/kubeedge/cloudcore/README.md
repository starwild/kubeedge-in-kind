...
sh -c "helm upgrade --install cloudcore . --namespace kubeedge --create-namespace -f ./values.yaml --set cloudCore.modules.cloudHub.advertiseAddress[0]=1.1.1.1"
...
# namespace内有证书，删除后不可复原，边缘节点无法连接，谨慎处理。 kubectl_delete_ns_kubeedge

kubectl get crds  | egrep "devices\.kubeedge.io\s" | awk '{print $1}' | xargs -I{} kubectl delete crds {}

kubectl -n kubeedge delete configmap tunnelport
